...
def train(args):
    ...
    # Set model
    model = DenseNetClassifier(num_classes=args.num_classes, pretrained=args.use_imagenet_weights)  # Change to your classification model
    # model = torch.compile(model)
    model.cuda()
    print(tuple(model._modules)[-1])
        ...
        for data, label in tqdm(train_loader):
            ...
            if epoch < args.warmup_epochs and args.use_imagenet_weights:
                ...
                for name, param in model.named_parameters():
                    if (name != f"{tuple(model._modules)[-1]}.classifier.weight") and (name != f"{tuple(model._modules)[-1]}.classifier.bias"):
